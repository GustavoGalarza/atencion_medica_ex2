...
<body>
    ...
    <form method="post" action="eliminar_planes.php">
        <h2>Eliminar plan de tratamiento de paciente</h2>
        <label for="ID_PlanTratamiento">Id del plan de tratamiento a eliminar:</label>
        <select name="ID_PlanTratamiento" id="ID_PlanTratamiento">
            <?php
            ...
            $select_query = "SELECT ID_PlanTratamiento FROM planestratamiento";
            $result = mysqli_query($mysqli_link, $select_query);

            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                echo '<option value="' . $row['ID_PlanTratamiento'] . '">' . $row['ID_PlanTratamiento'] . '</option>';
            }
