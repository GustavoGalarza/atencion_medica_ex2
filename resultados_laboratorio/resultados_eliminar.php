...
<body>
    ...
    <div class="contenido">
        <form method="post" action="eliminar_resultado.php">
            <h2>Eliminar resultado de laboratorio</h2>
            <label for="ID_Resultado">Id del resultado de laboratorio a eliminar:</label>
            <select name="ID_Resultado" id="ID_Resultado">
                ...
                $select_query = "SELECT ID_Resultado FROM resultadoslaboratorio";
                $result = mysqli_query($mysqli_link, $select_query);

                while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                    echo '<option value="' . $row['ID_Resultado'] . '">' . $row['ID_Resultado'] . '</option>';
                }
